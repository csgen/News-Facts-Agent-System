...
def run_eval(
    ...
    for i, record in enumerate(records):
        ...
        if three_way:
            truth = record.ground_truth_verdict or (
                "supported" if record.ground_truth_binary == 0 else "refuted"
            )
            y_true.append(truth)
        ...
        result_rows.append(
            {
                ...
                "correct": (
                    pred_label == record.ground_truth_verdict
                    if three_way
...
def main():
    ...
    parser.add_argument(
        "--factify2-path", help="Path to Factify2 CSV file (train.csv / val.csv / test.csv)"
    )
