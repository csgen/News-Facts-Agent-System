...
class FactCheckInput(BaseModel):
    ...
    )  # pre-decomposed sub-claims from preprocessing; empty = single-claim mode
    ...
    timestamp: datetime
    prefetched_chunks: list[str] = Field(
        default_factory=list
    )  # pre-fetched evidence; skips live_search when non-empty
    topic_text: str = (
        ""  # coarse topic from ClaimIsolator; empty string for frontend/benchmark paths
    )
