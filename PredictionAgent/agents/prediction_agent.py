...
def generate_prediction_text(
    entity_name: str, trend: dict, snapshots: list[dict]
) -> tuple[str, float]:
    """
    ...
    """
    scores_str = ", ".join(f"{float(s['credibility_score']):.2f}" for s in snapshots[-8:])
...
def _rule_based_prediction(entity_name: str, trend: dict) -> str:
    ...
    if d == "RISING":
        ...
        return (
            f"{entity_name}'s credibility has been rising steadily "
        ...
        return (
            f"{entity_name}'s credibility is declining "
        ...
        return (
            f"{entity_name} shows high credibility volatility "
        ...
        return (
            f"{entity_name}'s credibility has been stable at approximately "
...
def run_prediction_agent(
    entity_name: str, memory: Optional[MemoryAgent] = None
) -> Optional[Prediction]:
    """
    ...
    print(
        f"[prediction_agent] Trend={trend['direction']}  "
        f"slope={trend['slope']:+.5f}  R²={trend['r_squared']:.3f}  "
        f"vol={trend['volatility']:.4f}  "
        f"{trend['start']:.2f}→{trend['current']:.2f}"
    )

    prediction_text, confidence = generate_prediction_text(entity_name, trend, snapshots_sorted)
    print(f"[prediction_agent] confidence={confidence:.0%}  text={prediction_text[:80]!r}...")
