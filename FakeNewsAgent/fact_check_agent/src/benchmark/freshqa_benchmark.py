...
def load_freshqa_csv(path: str) -> pd.DataFrame:
    ...
    if not q_col or not c_col:
        raise ValueError(
            f"Cannot find question/category columns in {path}. Available: {list(df.columns)}"
        )
...
def run_benchmark(
    ...
    if df.empty:
        raise ValueError(
            f"No rows match categories {categories}. Available: {df['category'].unique().tolist()}"
        )
    if sample:
...
def main() -> None:
    ...
    parser.add_argument(
        "--stale-days", type=int, default=365, help="Simulated cache age in days (default: 365)"
    )
    parser.add_argument(
        "--categories", default="never-changing,slow-changing,fast-changing,false-premise"
    )
    parser.add_argument(
        "--sample",
        type=int,
    ...
    parser.add_argument("--no-save", action="store_true", help="Skip saving per-model results CSV")

    # Langfuse options
    lf_group = parser.add_argument_group("Langfuse (optional)")
    lf_group.add_argument(
        "--langfuse", action="store_true", help="Log traces and scores to Langfuse"
    )
    lf_group.add_argument(
        "--dataset-name", default="freshqa", help="Langfuse dataset name (default: freshqa)"
    )
    lf_group.add_argument(
        "--run-name",
        default=None,
        help="Base run name; model name is appended per run. Default: freshness-<stale_days>d",
    )
    lf_group.add_argument(
        "--no-upload",
        action="store_true",
