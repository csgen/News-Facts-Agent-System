...
def get_system_verdict(claim_text: str, image_url: Optional[str] = None) -> dict:
    ...
    try:
        ...
        label = output.verdict if output.verdict in ("supported", "refuted", "misleading") else "misleading"
        return {
            "label":          label,
            "confidence":     round(output.confidence_score / 100, 2) if output.confidence_score > 1 else round(output.confidence_score, 2),
...
def get_retrieved_claims(query: str, k: int = 5) -> list[dict]:
    ...
    try:
        ...
        for i, doc in enumerate(results.get("documents", [[]])[0]):
            ...
            retrieved.append({
                "claim_id":    results.get("ids", [[]])[0][i] if results.get("ids") else f"clm_{i}",
...
def compute_entity_f1(y_true_entities: list[list[str]],
                      y_pred_entities: list[list[str]]) -> dict:
...
def load_liar_dataset(n_samples: int = 100) -> list[dict]:
    ...
    try:
        ...
        liar_map = {
            "true": "supported", "mostly-true": "supported", "half-true": "supported",
...
def main():
    ...
    parser.add_argument("--dataset", choices=["liar", "fever", "factify", "all"],
                        default="all", help="Which dataset to evaluate on")
