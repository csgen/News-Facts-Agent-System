...
def speaker_credibility(row) -> float:
    ...
    total = (
        row.barely_true_count
        + row.false_count
        + row.half_true_count
        + row.mostly_true_count
...
class BenchmarkRecord(BaseModel):
    ...
    ground_truth_verdict: Optional[str] = (
        None  # "supported" | "misleading" | "refuted" (Factify2 only)
    )
...
def load_fakenewsnet_article(
    ...
    content_hash = hashlib.sha256(f"{url}{title}{body_text[:200]}".encode()).hexdigest()
    ...
    for _, row in df.iterrows():
        ...
        records.append(
            BenchmarkRecord(
                ...
                source_url=(
                    f"https://www.politifact.com/personalities/{speaker.lower().replace(' ', '-')}/"
                ),
                ...
                split=split,
                speaker=speaker,
                speaker_job_title=str(row.speaker_job_title)
                if pd.notna(row.speaker_job_title)
                else None,
                party_affiliation=str(row.party_affiliation)
